...
#' @param drop_pos Optional Korean POS tags to remove before n-gram generation.
...
#' @return A list of character vectors, named when the input is named. Empty or
#'   too-short documents return `character(0)`; missing documents return
#'   `NA_character_`.
#' @examples
#' \dontrun{
...
token_ngrams <- function(phrase, n = 3L,
                         ...
                         skip = 0L, keep_pos = NULL, drop_pos = NULL,
  ...
  grams <- simple_ngrams(tokens, n, skip, stopwords, enc2utf8(ngram_delim))
  .simplify_list(grams, .check_flag(simplify, "simplify"))
}
