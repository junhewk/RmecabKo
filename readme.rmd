...
*현재 수정중입니다. 곧 공개하도록 하겠습니다.* 한국어 문장 분석을 위해서는 `mecab-ko-dic` 또한 설치해야 합니다. `mecab-ko-dic` 설치를 위한 `install_dic` 함수를 추가했습니다(현재 Github version에만 적용). 현재 사용자 사전 기능 추가를 위해 작업 중이며, 사용사 사전을 사용하기 위해서는 `mecab-ko-dic` source를 `make`를 통해 build 해놓아야 합니다. `install_dic`은 package directory에 `mecab-ko-dic` 최신 버전을 받아 시스템에 설치하며, 이전의 `mecab-ko-dic`은 삭제합니다. 추후 Windows 10 Linux subsystem에서도 동일하게 작동할 수 있도록 확장할 예정입니다.
