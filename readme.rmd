...
*`RmecabKo`의 다국어 버전 `RcppMeCab`을 [Github](https://github.com/junhewk/RcppMeCab)에 업로드했습니다.* 윈도에서 네이티브로 설치 가능하며 인코딩 문제를 C++ 단에서 해결했습니다. 곧 `RmecabKo`의 메인 모듈을 `RcppMeCab`에서 inherit하는 것으로 변경할 예정입니다. 감사합니다. 
